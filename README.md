...
- **端口自适应**：使用 `--port 0` 由系统分配空闲端口，不会和已运行的 `dsh web`（默认 3080）冲突。
- **开启动画**：启动时中间先显示加载环，服务就绪后变成一道「门」——中间是圆形鲸鱼图标，点击图标门从中间上下打开并伴有蓝色光效，进入应用。
- **外观自定义**：菜单「设置 → 外观设置」（`Ctrl+,`）可自定义开门光效颜色、开门图标、界面背景图片、主界面壁纸（上传高清图替换）。
- **识图能力（modlens）**：内置 [ModLens](https://github.com/liustack/modlens) 视觉插件，纯文本模型也能读图（OCR/版面/语义），安装后即可粘贴图片识别。
...
- **用户数据**：`~/.dsh`（与 CLI 版共享）。
- **运行日志**：Electron 的 `logs` 目录（Windows：`%APPDATA%\deepseek-harness-desktop\logs`），其中 `dsh-web.log` 记录了 dsh 子进程输出，排障时优先看这里。
